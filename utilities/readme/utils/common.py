"""Common utilities for the README generation."""
...
def get_all_destinations(ext: str = "*") -> list[str]:
    """Return a list of all the destinations across all repositories.

    Args:
        ext (str): The extension to filter by. Defaults to all files.

    Returns:
        list: A list of all the destinations across all repositories.
    """
    all_destinations: set[str] = set()

    for mappings in REPO_FILE_MAPPINGS.values():
        if ext.startswith("."):
            all_destinations.update(dest for dest in mappings if dest.endswith(ext))
        else:
    ...
    if source.is_file():
        if excluded_files:
            raise InvalidMappingError(
                entry,
                reason="Cannot exclude files from a single file",
            )
    ...
    if source.is_dir():
        for efile in excluded_files:
            if not (source / efile).is_file():
                raise InvalidMappingError(
                    entry,
                    reason=f"Excluded file `{efile}` does not exist",
                )
